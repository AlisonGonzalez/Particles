...
#define CROWD_PARSER_MAX_CAMS 8

Camera _crowd_cams[CROWD_PARSER_MAX_CAMS];
int _crowd_num_cams=0;
int _crowd_current_cam=0;
...
Camera *Camera::getCurrentCamera(){
	return &_crowd_cams[_crowd_current_cam];
}

Camera *Camera::getCameraAt(int index){
	return &_crowd_cams[index];
}

Camera *Camera::getNextCamera(){
	if(_crowd_num_cams < CROWD_PARSER_MAX_CAMS){
		return &_crowd_cams[_crowd_num_cams++];
	} else {
		printf("Error: Maximum number of cameras exceeded\n");
		return &_crowd_cams[_crowd_num_cams-1];
	}
}

int Camera::getNumCameras(){
	return _crowd_num_cams;
}

void Camera::reset(){
	_crowd_num_cams=0;
	_crowd_current_cam=0;
}
...
void Camera::updatePlanes (float *X, float *Y, float *Z, float &ratio_) 
{
	...
	crossProduct (this->Z, up, this->X);
	normalize (this->X);

	// the real "up" vector is the dot product of X and Z
	crossProduct (this->X, this->Z, this->Y);
	normalize(this->Y);
